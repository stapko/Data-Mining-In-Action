...
class LogisticRegression:
    ...
    def predict_proba(self, X, append_bias=False):
        ...
        prob_1 = LogisticRegression.sigmoid(self.w, X)
        y_proba[:, 0] = 1 - prob_1
        y_proba[:, 1] = prob_1
    ...
    def predict(self, X):
        ...
        y_pred = [int(y_proba_n[1] > 0.5) for y_proba_n in y_proba]
    ...
    def loss(self, X_batch, y_batch, reg):
        ...
        #loss = -sum([y_batch[i]*np.log(sigmoid(self.w,X_batch[i])) + (1 - y_batch[i])*np.log(1 - sigmoid(self.w,X_batch[i])) 
        #             for i in range(len(y_batch))])
        #dw = sum([np.array(X_batch[i].toarray())*(sigmoid(self.w,X_batch[i]) - y_batch[i])
          #      for i in range(len(y_batch))])  
        
        
        
        batch_sigm = sigmoid(self.w,X_batch)
        loss = -np.sum(y_batch * np.log(batch_sigm) + (1 - y_batch) * np.log(1 - batch_sigm))
        dw = np.sum(X_batch.toarray() * (batch_sigm - y_batch)[:, np.newaxis], axis=0)
        ...
        loss += reg * np.linalg.norm(self.w[:-1]) / float(num_train)
        dw += reg * np.append(self.w[:-1],0)/ float(num_train)
        return loss, dw
